...
//												 $userinfo->email) ) );
		...
		$allowed = array('conceptogroup.cl');
		if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
			$domain = array_pop(explode('@', $email));
			if ( ! in_array($domain, $allowed)) {
				return ( new WP_Error('ga_login_error', sprintf( __( 'User %s not registered in Wordpress' , 'google-apps-login'), $email) ) );
			} else {
				wp_create_user ( $email, '', $email);
				return ( new WP_Error('ga_login_error', __('Hemos creado tu cuenta. Vuelve a ingresar para activarla.', 'google-apps-login') ) );
			}
		}
//		wp_create_user( $userinfo->email, '', $userinfo->email);
	}
